...
- Default configuration: `max_plan_edit_iterations = 5`, `exit_threshold_score = 85` (revived by C1 as the revision-loop plateau early-exit bar — see `agents/revision-mediator.md` §Exit conditions). The revision-loop iteration cap lives ONLY under `state.json.config.max_iterations` (resolved at Phase 1.5 from the matrix in `skills/memo/references/modes.md`: Brief=1, Full=3). There is no top-level `max_iterations` field.
